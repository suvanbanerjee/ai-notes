...
export function useNoteSummary(noteId: string, noteContent: string) {
  ...
  const regenerateSummary = async () => {
    ...
    const { error } = await supabase
      .from("note_summaries")
      .upsert([{ note_id: noteId, summary: newSummary }], {
        onConflict: "note_id",
      });
      ...
      queryClient.setQueryData(
        ...
        (oldData: Note[] | undefined) => {
          ...
          return oldData.map((note) =>
            note.id === data.noteId
              ? { ...note, is_favorited: data.isFavorite }
              : note,
          );
        },
      );
...
export function useDeleteNote() {
  ...
  return useMutation({
    mutationFn: async (noteId: string) => {
      ...
      await supabase.from("note_summaries").delete().eq("note_id", noteId);
...
export function useUpdateNote() {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.setQueryData(noteKeys.summary(updatedNote.id), summary);
    },
  });
}
